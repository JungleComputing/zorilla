...
public final class DumpServlet extends HttpServlet {

    private static final long serialVersionUID = 1L;

    private static Logger logger = Logger.getLogger(DumpServlet.class);
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (info != null && info.endsWith("Exception")) {
            try {
                throw (Throwable) (Loader.loadClass(this.getClass(), info
                        .substring(1)).newInstance());
            } catch (Throwable th) {
        ...
        try {
            ...
            table.newRow();
            table.newHeading().cell().nest(new Font(2, true)).add(
                    "<BR>Other HTTP Headers").attribute("COLSPAN", "2").left();
            ...
            table.newRow();
            table.newHeading().cell().nest(new Font(2, true)).add(
                    "<BR>Request Parameters").attribute("COLSPAN", "2").left();
            ...
            table.newRow();
            table.newHeading().cell().nest(new Font(2, true)).add(
                    "<BR>Request Attributes").attribute("COLSPAN", "2").left();
            Enumeration a = request.getAttributeNames();
            while (a.hasMoreElements()) {
                name = (String) a.nextElement();
                table.newRow();
                table.addHeading(name + ":&nbsp;").cell().attribute("VALIGN",
                        "TOP").right();
            ...
            table.newRow();
            table.newHeading().cell().nest(new Font(2, true)).add(
                    "<BR>Servlet InitParameters").attribute("COLSPAN", "2")
                    .left();
            ...
            table.newRow();
            table.newHeading().cell().nest(new Font(2, true)).add(
                    "<BR>Context InitParameters").attribute("COLSPAN", "2")
                    .left();
            ...
            table.newRow();
            table.newHeading().cell().nest(new Font(2, true)).add(
                    "<BR>Context Attributes").attribute("COLSPAN", "2").left();
            ...
            if (request.getContentType() != null
                ...
                table.newRow();
                table.newHeading().cell().nest(new Font(2, true)).add(
                        "<BR>Multi-part content").attribute("COLSPAN", "2")
                        .left();
                for (int p = 0; p < parts.length; p++) {
                    name = parts[p];
                    table.newRow();
                    table.addHeading(name + ":&nbsp;").cell().attribute(
                            "VALIGN", "TOP").right();
            ...
            if (res != null && res.length() > 0) {
                table.newRow();
                table.newHeading().cell().nest(new Font(2, true)).add(
                        "<BR>Get Resource: " + res).attribute("COLSPAN", "2")
                        .left();
            ...
            page.add("Directly encoced:  D??rst<br/>");
            page.add("HTML reference: D&uuml;rst<br/>");
            page.add("Decimal (252) 8859-1: D&#252;rst<br/>");
            page.add("Hex (xFC) 8859-1: D&#xFC;rst<br/>");
            page
                    .add("Javascript unicode (00FC) : <script language='javascript'>document.write(\"D\u00FCrst\");</script><br/>");
            page.add(Break.para);
        ...
        if (data != null && data.length() > 0) {
            ...
            while (d > 0) {
                pout
                        .println("1234567890123456789012345678901234567890123456789\n");
                d = d - 50;
