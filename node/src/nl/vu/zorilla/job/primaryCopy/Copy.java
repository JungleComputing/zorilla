...
public final class Copy extends Job implements Receiver, Runnable {
    ...
    public Copy(Advert advert, Node node) throws IOException,
            Exception {
        ...
        ibis = createIbis(jobID.toString());

        callPortType = createCallPortType(ibis);
        replyPortType = createReplyPortType(ibis);
        endPoint = new EndPoint(id.toString(), this, callPortType,
                replyPortType);
    ...
    public void cancel() throws Exception {
        throw new Exception("cancelation of job "
                + "only possible at node where job was submitted");
    }

    @Override
    public
    synchronized void end(long deadline) {
    ...
    @Override
    public
    synchronized boolean zombie() {
        return ended;
    }

    @Override
    public
    synchronized Resources usedResources() {
    ...
    public synchronized Map<String, String> getStats() {
        ...
        if (initialized) {
            ...
            result.put("Local.workers", String.valueOf(localWorkers
                    .size()));
    ...
    private synchronized void handleCreateWorkers(Invocation invocation)
        ...
        for (int i = 0; i < nrOfWorkers; i++) {
            if (!node.config().booleanProperty(Config.NATIVE_JOBS) && isNative()) {
    ...
    private boolean initialize() throws IOException, Exception {
        ...
        synchronized (this) {
            try {
                ...
                throw new Exception("could not read state"
                        + " from call reply", e);
            }
            ...
            if ((scheme == null || !scheme.equalsIgnoreCase("java"))
                    && !node.config().booleanProperty(Config.NATIVE_JOBS)) {
                throw new Exception(
    ...
    private void sendMaxNrOfWorkers() throws IOException, Exception {
        ...
        synchronized (this) {
            ...
            nrOfWorkers = node.jobService().nrOfResourceSetsAvailable(workerResources)
                    + localWorkers.size();
