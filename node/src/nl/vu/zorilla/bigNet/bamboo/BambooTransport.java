...
public final class BambooTransport implements Runnable {
    ...
    private final InetSocketAddress homeAddress;
    ...
    public BambooTransport(Node node, BambooNetwork network, InetSocketAddress address)
        ...
        InetSocketAddress[] peers = node.config().getPeers(); 
        ...
        homeAddress = node.config().getHomePeer();
    ...
    public void run() {
        try {
            ...
            if (homeAddress != null) {
                String[] phonehomeArgs = new String[3];
                phonehomeArgs[0] = nodeID[0];
                phonehomeArgs[1] = "home_address="
                    + homeAddress.getAddress().getHostAddress();
                phonehomeArgs[2] = "home_port=" + homeAddress.getPort();

                sandstormConfig.addStage("PhoneHomeStage",
                    "nl.vu.zorilla.bigNet.bamboo.PhoneHomeStage", phonehomeArgs);
            }

            // zorilla interface
            sandstormConfig.addStage("Zorilla",
                "nl.vu.zorilla.bigNet.bamboo.BambooStage", nodeID);
