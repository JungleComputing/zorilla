...
public final class Worker implements Runnable {
    ...
    public enum Status {
        ...
        public static Status fromOrdinal(int ordinal) throws ZorillaException {
            for (Status status : Status.class.getEnumConstants()) {
                if (status.ordinal() == ordinal) {
                    return status;
                }
            }
            throw new ZorillaException("unknown status: " + ordinal);
        }
    }
    ...
    private ProcessBuilder nativeCommand(File workingDir)
        ...
        // script which runs program. needed in unix because given executable
        // may be in input files of job and not have executable status
        // result.add(zorillaHome + fileSeparator + "bin" + fileSeparator
        // + "run-native");
    ...
    private ProcessBuilder javaCommand(File workingDir) throws ZorillaException {
        String appClassPath;
        ProcessBuilder result = new ProcessBuilder();
        ...
        result.command().add(
                "-Djava.security.policy==file:" + zorillaHome + File.separator
        ...
        // zorilla home
        result.command().add("-Dzorilla.home=" + zorillaHome);
    ...
    private File createScratchDir(UUID id) throws IOException, ZorillaException {
    ...
    public void run() {
        ...
        node.message("starting worker " + this + " for " + job);

        try {
            log = job.createLogFile(id().toString() + ".log");
        } catch (Exception e) {
            logger.error("could notcreate log file", e);
        ...
        try {
            ...
            if (job.isNative() && !node.config().nativeJobs()) {
                throw new ZorillaException(
            ...
            while (status().equals(Status.RUNNING)) {
                try {
                    ...
                    node.message("worker " + this + "(" + status()
            ...
            node.warn("error on running worker", e);
            try {
                setStatus(Status.ERROR);
                log.printlog(e.toString());
                job.flush();
