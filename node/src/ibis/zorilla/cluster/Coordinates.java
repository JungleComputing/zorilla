...
public final class Coordinates implements Serializable {
    ...
    public static final int SIZE = SizeOf.DOUBLE + (SizeOf.DOUBLE * DIMENSIONS);
    ...
    public Coordinates(byte[] bytes) throws IOException {
        ...
        for (int i = 0; i < DIMENSIONS; i++) {
            long coordinateBits = byte2long(bytes, SizeOf.LONG * (i + 1));
    ...
    public byte[] toBytes() {
        ...
        for (int i = 0; i < DIMENSIONS; i++) {
            ...
            long2byte(coordinateBits, bytes, SizeOf.LONG * (i + 1));
        }
        return bytes;
    }
