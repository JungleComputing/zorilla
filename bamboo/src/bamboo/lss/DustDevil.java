...
public class DustDevil {
    ...
    public void init_nodes(SandstormConfig config, StageManager mgr) {
        ...
        while (iter.hasMoreElements()) {
            ...
            try {
                ...
                logger.fatal("Error: caught exception " + e
                    + " while trying to instantiate " + "an object of type "
                    + descr.className + ".", e);
                System.exit(1);
            }

            SinkIF sink = new DustDevilSink(handler, Thread.currentThread());
            StageIF stage = new SimulatorStage(descr.stageName,
                new StageWrapper(handler), sink);
        ...
        while (!handlers.isEmpty()) {
            ...
            try {
                ...
                logger.fatal("Error: caught an exception " + e
                    + " while trying to initialize the stage " + handler + ".",
                    e);
                System.exit(1);
            }
        }
    ...
    protected void create_network_stage(String name, String node_id_str,
        ...
        InetSocketAddress my_addr = new InetSocketAddress(node_id.address(),
            node_id.port());

        EventHandlerIF network = create_network(my_addr);
        SinkIF sink = new DustDevilSink(network, Thread.currentThread());
        StageIF stage = new SimulatorStage(name, new StageWrapper(network),
            sink);
        config_data.setStage(stage);
    ...
    public StageManager main(SandstormConfig cfg) throws Exception {
        ...
        while (iter.hasMoreElements()) {
            ...
            if (descr.stageName.indexOf("YANetwork") != -1) {
                ...
                // nidstr might be null now...

                create_network_stage(descr.stageName, nidstr, mgr, config_data,
                    classifiers);
            }
        }
